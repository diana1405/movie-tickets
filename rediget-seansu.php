...
<html lang="lv">
    ...
    <header>
        ...
        <h1>Rediģēt seansu</h1>
        

        <!--SELECT-->
        <div id="container2">
        <?php $id = $_GET["id"]; ?>
            <form id="select" action="rediget-seansu-check.php?id=<?php echo $_GET['id'];?>" method="post">
                <div class="custom-select" id="datums">
                    ...
                    <select name="film-name" id="town" required>
                      ...
                      $old_info_sql = $mysql->query("SELECT `Nosaukums`, `Datums`, `No`, `TelpaID`, `filmas`.`FilmaID` FROM `filmas` 
                      INNER JOIN `seansi` ON `filmas`.`FilmaID` = `seansi`.`FilmaID`
                      WHERE `SeanssID` = $id");
                      $old_info = $old_info_sql -> fetch_assoc();?>

                      <option value="<?php echo $old_info['FilmaID'] ?>"><?php echo $old_info['Nosaukums'] ?></option>
                      <?php
                ...
                <div class="label">  
                    <p>Datums:</p>
                    <input type="date" name="date" id="date" placeholder="Datums" value="<?php echo $old_info['Datums'] ?>" required>
                </div>
                <div class="label">
                    <p>Laiks:</p>
                    <input type="time" name="time" id="time" placeholder="Laiks" value="<?php echo $old_info['No'] ?>" required>
                </div>
                <div class="custom-select" id="datums">
                    <label for="telpa">Telpa:</label>
                    <select name="telpa" id="town" required>
                      <option value="<?php echo $old_info['TelpaID'] ?>"><?php echo $old_info['TelpaID'] ?></option>
                ...
                <div id="buy-ticket">
                    <a href=""><button type="submin">Rediģēt</button></a>
                </div>
            </form>
        </div>
